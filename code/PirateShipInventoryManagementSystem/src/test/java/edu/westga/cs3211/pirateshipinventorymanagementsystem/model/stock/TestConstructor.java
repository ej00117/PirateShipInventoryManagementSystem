...
class TestConstructor {
	...
	void testNullStockName() {
		assertThrows(IllegalArgumentException.class, ()->{new Stock(null, 2.0, Condition.PERFECT, new ArrayList<SpecialQuality>());});
	}
	
	@Test
	void testNullStockQuantity() {
		assertThrows(IllegalArgumentException.class, ()->{new Stock("gold", null, Condition.PERFECT, new ArrayList<SpecialQuality>());});
	}
	
	@Test
	void testQuantityAtOrBelowZero() {
		assertThrows(IllegalArgumentException.class, ()->{new Stock("gold", 0.0, Condition.PERFECT, new ArrayList<SpecialQuality>());});
	}
	
	@Test
	void testNullStockCondition() {
		assertThrows(IllegalArgumentException.class, ()->{new Stock("gold", 2.0, null, new ArrayList<SpecialQuality>());});
	}

	@Test
	void testNullStockSpecialQualities() {
		assertThrows(IllegalArgumentException.class, ()->{new Stock("gold", 2.0, Condition.PERFECT, null);});
	}
	...
	void testGiveRegularStockPerishableQuality() {
		...
		assertThrows(IllegalArgumentException.class, ()->{new Stock("gold", 2.0, Condition.PERFECT, qualities);});
	}
